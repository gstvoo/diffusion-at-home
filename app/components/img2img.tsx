...
export default function ImageToImage({model, title, initialImage, initialPrediction} : Image2ImageProps) {
  ...
  useEffect(() => {
    ...
    const storedImage = sessionStorage.getItem('image'+model);
    if (storedPrediction) {
  ...
  const handleImageUpload = (e : any) => {
    ...
    reader.onloadend = function() {
        ...
        sessionStorage.setItem('image'+model, image);
    }
